...
fscanf(in, "%d %s", &nopcion, trash); /* Se selecciona si el programa continúa o inciai por primera vez  */
...
fscanf(in, "%lf %s", &time, trash); /* Tiempo de integración   */
fscanf(in, "%lf %lf %lf %s", &boxx, &boxy, &boxz, trash); /* Lados de la caja de simulación (Lx, Ly, Lz) */
fscanf(in, "%d %d %d %s", &ndivx, &ndivy, &ndivz, trash); /* Número de átomos por lado de la caja (nx, ny, nz) */
...
fscanf(in, "%lf %s", &presion, trash); /* Presión para presostato */
...
fscanf(in, "%d %s", &nconfequi, trash); /* Número de configuraciones para equilibrar   */
fscanf(in, "%d %s", &nconf, trash); /* Número de configuraciones  */
...
fscanf(in, "%d %s", &nmovie, trash); /* Frecuencia de fotos para película de GROMACS*/
...
if (nopcion == 1){
   crear_posiciones(&nat, &rho, dofx, rx, ry, rz, boxx, boxy, boxz, ndivx, 
   ndivy, ndivz);
