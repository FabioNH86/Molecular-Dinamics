...
hoomd.context.initialize("--mode=gpu")# --gpu=1")
...
#   system=hoomd.init.create_lattice(unitcell=hoomd.lattice.sq(a=separacion),n=[260,130]);
   system=hoomd.init.create_lattice(unitcell=hoomd.lattice.hex(a=separacion),n=[130,130]);
...
if noption == 0:
   luigi=md.integrate.langevin(group=all, kT=temperatura, seed=5)
   hoomd.run(1e3)
   luigi.disable()

#md.integrate.nve(group=all)
md.integrate.nvt(group=all, kT=temperatura, tau=1.0)
