...
def correr_simulacion_homoplimero(snapshot, temp, equilibracion, muestreo, mon_cadena, aspect_ratio, eps_SP=1.0):
    ...
    # Para poder guardar la primer configuración en el gsd y ver cómo se acomodaron las partículas
    timestep_inicio_muestreo = sim.timestep  # justo después de sim.run(equilibracion)

    trigger_combinado = hoomd.trigger.Or([
        hoomd.trigger.On(timestep_inicio_muestreo),
        hoomd.trigger.On(timestep_inicio_muestreo + 1),
    ...
    pressure_tensor_log = hoomd.logging.Logger(categories=['sequence'])
...
def continue_sim_from_gsd(archivo_gsd, temp, eps_SP, mon_cadena, aspect_ratio,
                           pasos_equilibracion=2_500_000, pasos_muestreo=3_000_000):
    file_id = f"Poly-Solv_T{temp:.2f}_epsSP{eps_SP:.2f}"
    ...
    # --- Fuerzas e integrador (siempre necesarios, sin importar la fase) ---
    cell = hoomd.md.nlist.Cell(buffer=0.4)
    lj = hoomd.md.pair.LJ(nlist=cell, default_r_cut=4.0, mode='shift')
    lj.params[('S', 'S')] = dict(epsilon=1.0, sigma=1.0)
    lj.params[('P', 'P')] = dict(epsilon=1.0, sigma=1.0)
    ...
    # --- Logger escalar: útil en ambas fases, así que va desde ya ---
    logger = hoomd.logging.Logger(categories=['scalar'])
    logger.add(thermo, quantities=['potential_energy', 'kinetic_energy', 'kinetic_temperature', 'pressure'])
    table = hoomd.write.Table(trigger=hoomd.trigger.Periodic(10000),
                               logger=logger,
    ...
    # ------------------------------------------------------------------
    # Aquí es donde se estira la caja
    # ------------------------------------------------------------------
    original_box = sim.state.box

    if original_box.Lx > 100.0:
        print(f"📦 La caja ya está estirada ([{original_box.Lx:.2f}, "
              f"{original_box.Ly:.2f}, {original_box.Lz:.2f}]).")
    else:
        print("📦 Estirando la caja para la fase de muestreo...")
        new_box = hoomd.Box(
            Lx=original_box.Lx * aspect_ratio,
            Ly=original_box.Ly + 0.1,
            Lz=original_box.Lz + 0.1,
            xy=original_box.xy,
            xz=original_box.xz,
            yz=original_box.yz
        )
        sim.state.set_box(box=new_box)
        print(f"Se estiró la caja a: \n{[new_box.Lx, new_box.Ly, new_box.Lz]}")

    # ------------------------------------------------------------------
    # FASE 1: Equilibración (si aún no llegamos a pasos_equilibracion)
    # ------------------------------------------------------------------
    current_step = sim.timestep
    print(f"📍 Retomando desde el timestep {current_step}")

    if current_step < pasos_equilibracion:
        faltan_equilibracion = pasos_equilibracion - current_step
        print(f"🔄 Aún en equilibración. Corriendo {faltan_equilibracion} pasos más "
              f"hasta llegar a {pasos_equilibracion}...")
        sim.run(faltan_equilibracion)
    else:
        print(f"✅ Equilibración ya completada (timestep actual: {current_step}).")


    # ------------------------------------------------------------------
    # FASE 2: Muestreo (writers pesados solo se agregan aquí)
    # ------------------------------------------------------------------
    trigger_combinado = hoomd.trigger.Or([hoomd.trigger.On(0),
                                           hoomd.trigger.On(1),
                                           hoomd.trigger.Periodic(5000)])
    ...
    faltan_muestreo = pasos_muestreo - sim.timestep
    if faltan_muestreo > 0:
        print(f"🎯 Corriendo fase de muestreo: {faltan_muestreo} pasos "
              f"hasta llegar a {pasos_muestreo}...")
        sim.run(faltan_muestreo)
    else:
        print(f"Aviso: la simulación ya está en el paso {sim.timestep}, "
              f"no se necesita muestreo adicional.")
